...
final class ProjectWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(repoPath: String, first: Bool) {
        ...
        window.tabbingIdentifier = "pairprogram" // projects open as tabs of one window
    ...
    private func build(first: Bool) {
        ...
        if first {
            ...
            if UserDefaults.standard.object(forKey: "NSSplitView Subview Frames pairprogram.split.v2") == nil,
    ...
    private func makeSplit() -> NSSplitViewController {
        ...
        split.splitView.autosaveName = "pairprogram.split.v2" // three panes now; older saved widths don't apply
        return split
    }
    ...
    private func showRight(_ tab: SidebarController.Tab) {
        if commentsItem?.isCollapsed == true {
            ...
            UserDefaults.standard.set(false, forKey: "pairprogram.commentsHidden")
        }
