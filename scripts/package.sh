#!/usr/bin/env bash
# Builds PairProgram.dmg: a universal (Apple Silicon + Intel) app, signed with
...
DIST="$ROOT/dist"
APP="$DIST/PairProgram.app"
DMG="$DIST/PairProgram-$VERSION.dmg"
step() { printf '\n==> %s\n' "$1"; }
...
step "Assemble PairProgram.app"
rm -rf "$DIST" && mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BIN/pairprogram" "$APP/Contents/MacOS/PairProgram"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key><string>PairProgram</string>
  <key>CFBundleDisplayName</key><string>PairProgram</string>
  <key>CFBundleIdentifier</key><string>com.timwheeler.pairprogram</string>
  <key>CFBundleExecutable</key><string>PairProgram</string>
...
RW="$DIST/rw.dmg"
hdiutil create -quiet -volname "PairProgram" -srcfolder "$STAGE" -fs HFS+ -format UDRW -ov "$RW"
...
tell application "Finder"
  tell disk "PairProgram"
    ...
    set position of item "PairProgram.app" of container window to {180, 190}
